...
10. 查看「押金账本」，确认有 `deposit_deduct` 记录，冻结的押金正式扣除

**预期结果**:
- 预约状态正确流转: pending → approved → checked_in → completed
- 余额变化正确: 冻结时余额减少，核销时押金从冻结状态转为正式扣除
- 每一步都有操作历史记录和审计日志
...
- `GET /api/venues/export` - CSV 导出场地
...
- `GET /api/bookings/export` - CSV 导出预约
